...
module.exports = left = async (tobz, event) => {
    ...
    try {
        if (event.action == 'remove' && left) {
            ...
            const capt = ` @${event.who.replace('@c.us', '')} *telah meninggalkan group*\n═══════════════════\nSelamat jalan @${event.who.replace('@c.us', '')} hati-hati di jalan..\n*kalo keluar jangan lama lama nanti pacarmu di ambil orang nangis kau*🤣🤣\n═══════════════════\n 👋`
